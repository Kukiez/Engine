...
ParticleData spawnParticle(ParticleSpawner spawner, uint time) {
    ...
    particle.rotation = mix(spawner.minRotation, spawner.maxRotation, rng(seed));
    seed = pcg(seed);
    particle.rotationSpeed = mix(spawner.minRotationSpeed, spawner.maxRotationSpeed, rng(seed));
